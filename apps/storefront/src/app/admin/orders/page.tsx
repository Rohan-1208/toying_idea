...
import type { AdminOrder } from "@/app/admin/orders/AdminOrdersClient";
...
export default async function AdminOrdersPage() {
  ...
  const res = await fetch(await apiUrl("/api/admin/orders"), {
    cache: "no-store",
    headers: { cookie: jar.toString() },
  });
  if (res.status === 401 || res.status === 403) redirect("/admin");
  const orders = res.ok
    ? ((await res.json()) as AdminOrder[])
    : [];
      ...
      <div className="mx-auto w-full max-w-6xl px-5 py-10 grid gap-6">
        <div className="flex items-end justify-between gap-4">
          <div className="grid gap-2">
            ...
            <p className="text-sm text-muted">Orders stored in MongoDB.</p>
          </div>
